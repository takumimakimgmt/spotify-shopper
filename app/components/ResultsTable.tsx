...
import { categorizeTrack } from '../../lib/state/usePlaylistAnalyzer';
    ...
    <div className="hidden md:block mt-4 rounded-xl border border-slate-800 bg-slate-950/40 overflow-hidden">
      <div className="max-h-[70vh] overflow-auto">
        <table className="w-full text-xs table-fixed">
          ...
          <tbody>
            {sections.flatMap<React.ReactNode>((section) => {
              ...
              return [
                ...
                ...section.items.map((t) => {
                  ...
                  if (!recommended) {
                    return (
                      ...
                      >
                        ...
                        <td className="px-3 py-1 text-sm text-slate-100">{t.title}</td>
                        <td className="px-3 py-1 text-sm text-slate-300">{t.artist}</td>
                        <td className="px-3 py-1 text-xs text-slate-300">{t.album}</td>
                        <td className="px-2 py-1 text-xs text-slate-400 truncate">{t.isrc ?? ''}</td>
                        <td className="px-3 py-1 text-slate-500 text-[10px]">No store links</td>
                      </tr>
                    );
                  }
                  ...
                  return (
                    <tr
                      key={`${section.id}-${trackUrl ?? ''}-${t.index}-${t.isrc ?? ''}`}
                      className={`border-b border-slate-900/60 hover:bg-slate-800/40 transition-colors relative ${style.borderClass}`}
                    >
                      <td className="px-3 py-1 text-slate-400">{t.index}</td>
                      <td className="px-3 py-1 text-sm text-slate-100">
                        <a
                          ...
                          title={`${t.title}${style.tooltip ? ` (${style.tooltip})` : ''}`}
                        >
                          {t.title}
                        </a>
                      </td>
